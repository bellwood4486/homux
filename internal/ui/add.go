...
// まとめる」表現を踏襲する。
func RenderAddPlan(w io.Writer, home string, items []exec.AddItem) {
	...
	for _, it := range items {
		...
		fmt.Fprintf(w, "  -> %s\n", displayAbsPath(home, it.RepoPath))
		if it.Fork {
			fmt.Fprintln(w, "  (forks the existing common source)")
		}
	}
	fmt.Fprintln(w)
}
...
func RenderAddResult(w io.Writer, home string, res exec.AddResult) {
	if res.Err == nil {
